...
find . -name '*.cs' \
    ...
    -not -path './tests/*' \
    -print0 \
  | xargs -0 grep -hoE '(Console\.(Error\.)?(Write|WriteLine)|Ui\.(Info|Warn|Error|Success|Step|Detail|Raw|RawLine|Header|Confirm|ConfirmOrBack|Select|Ask)|SetupQuestionPrompt\.Ask)\(.*' \
  | sed -e 's/[[:space:]]\+$//' \
  | sort -u
