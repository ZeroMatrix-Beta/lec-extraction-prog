...
- **Flexible Extraction Options:** Toggle `GenerateOffsetFiles` to automatically generate timestamp-corrected LaTeX chunks, or use `GenerateAudioFile` to extract an MP3 of the entire lecture before processing.
...
- **Flexible Extraktions-Optionen:** Aktiviere `GenerateOffsetFiles` für automatisch zeitkorrigierte LaTeX-Stücke, oder nutze `GenerateAudioFile`, um vor der Verarbeitung eine komplette MP3 der Vorlesung zu exportieren.
