...
public class FfmpegInteractiveSession(FfmpegSessionConfig config) {
    ...
    /// </summary>
    private static string ShowMenuAndGetMode() {
        Console.WriteLine("\nConversion Options:");
        Console.WriteLine("\n--- Single File Options ---");
    ...
    private static bool IsValidMode(string mode) => int.TryParse(mode, out int m) && m >= 1 && m <= 12;

    private static string GetCustomCommandTemplate(out string outputExtension) {
    ...
    private static string[] SelectFilesToProcess(string sourceFolder, string mode) {
